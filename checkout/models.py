...
class Order(models.Model):
    ...
    real_items_total = models.DecimalField(
        max_digits=10, decimal_places=2, null=False, default=0)
    ...
    def update_total(self):
        ...
        if self.order_total > settings.DISCOUNT_THRESHOLD:
            self.discounted_total = self.order_total - (
                Decimal(
                    settings.STANDARD_DISCOUNT_PERCENTAGE / 100
                       ) * self.order_total
            )
        else:
            self.discounted_total = self.order_total
...
class TrackingOrder(models.Model):
    order = models.ForeignKey(
        Order, null=False, blank=False,
        on_delete=models.CASCADE, related_name='trackingOrder')
    tracking_number = models.CharField(
        max_length=64, null=False, editable=False)
    status = models.CharField(
        max_length=32, null=False, blank=False, default="On Route")
    location = models.CharField(
        max_length=32, null=False, blank=False, default="Mockville, MO")
